...
public class GeoConvert {
	...
	/**
	 * Implementacion de calculo de distancia entre 2 puntos GPS
	 */

	public static double distance2Points(double _lat1, double _long1, double _lat2, double _long2)
	{
		double dLat = convertRadianes(_lat2 - _lat1);
		double dLong = convertRadianes(_long2 - _long1);

		double a = Math.sin(dLat/2) * Math.sin(dLat/2) + Math.cos(convertRadianes(_lat1)) * Math.cos(convertRadianes(_lat2)) * Math.sin(dLong/2) * Math.sin(dLong/2);
		double c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
		return sm_a * c / 2;
	}

	private static double convertRadianes(double _x)
	{
